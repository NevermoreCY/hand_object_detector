...
if __name__ == '__main__':
  ...
  with torch.no_grad():
    ...
    folder_list = os.listdir(args.image_dir)
    out_dir_count = {}
    out_dir_conf = {}
    ...
    for folder_id in tqdm(range(len(folder_list))):
        ...
        while (num_images >= 0):
            ...
            # Load the demo image
              ...
              im_file = os.path.join(folder_path, imglist[num_images])
            ...
            if has_detect:
                ...
                for item in hand_dets:
                    # print(item[4])
                    confidence.append(float(item[4]))
        ...
        out_dir_count[folder] = total_count
        out_dir_conf[folder] = confidence

        # out_dir["total_count"] = total_count
        # out_dir["confidence"] = confidence
    ...
    # 使用with语句确保文件正确关闭
    file_path1 = os.path.join(args.save_dir, filename1)
    file_path2 = os.path.join(args.save_dir, filename2)
    with open(file_path1, 'w', encoding='utf-8') as file:
        # 将字典转换为json格式并写入文件
        json.dump(out_dir_count, file, ensure_ascii=False, indent=4)
    with open(file_path2, 'w', encoding='utf-8') as file:
        # 将字典转换为json格式并写入文件
        json.dump(out_dir_conf, file, ensure_ascii=False, indent=4)
